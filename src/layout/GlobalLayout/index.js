...
const GlobalLayout = (props) => {
    ...
    const [ openBar, setOpenBar ] = useSafeState(true)
    const [ openTypeBar, setTypeOpenBar ] = useSafeState(false)
    const [ openOtherPartyBar, setOtherPartyOpenBar ] = useSafeState(false)
    const [ openInventorBar, setInventorOpenBar ] = useSafeState(false)
    const [ openAssignmentBar, setAssignmentOpenBar ] = useSafeState(true) 
    const [ openCustomerBar, setCustomerOpenBar ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? true : false)
    const [ openIllustrationBar, setIllustrationBar ] = useSafeState(true)
    const [ openCommentBar, setCommentBar ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? true : false/* isMobile ? false : true */)
    const [ openChartBar, setChartBar ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? true : false)
    const [ openAnalyticsBar, setAnalyticsBar ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? true : false)
    const [ openVisualizerBar, setVisualizeOpenBar ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? true : false)

    const [ toggleButtonType, setToggleButtonType ] = useSafeState(true)
    const [ toggleTypeButtonType, setToggleTypeButtonType ] = useSafeState(true)
    const [ toggleOtherPartyButtonType, setToggleOtherPartyButtonType ] = useSafeState(true)
    const [ toggleAssignmentButtonType, setToggleAssignmentButtonType ] = useSafeState(true)
    const [ toggleCustomerButtonType, setToggleCustomerButtonType ] = useSafeState(true)  
    const [ securedTransactionAssets, setSecuredTransactionAssets ] = useSafeState(false)
    ...
    const [ companyBarSize, setCompanyBarSize ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? 0 : 210) 
    const [ typeBarSize, setTypeBarSize ] = useSafeState(0) 
    const [ otherPartyBarSize, setOtherPartyBarSize ] = useSafeState(0)
    const [ partyBarSize, setPartyBarSize ] = useSafeState('50%')
    const [ driveBarSize, setDriveBarSize ] = useSafeState('50%')
    const [ assignmentBarSize, setAssignmentBarSize ] = useSafeState(180)  
    const [ addressBarSize, setAddressBarSize ] = useState(450)
    const [ customerBarSize, setCustomerBarSize ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? 180 : 0)
    const [ commentBarSize , setCommentBarSize ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? '30%' : '0%')
    const [ illustrationBarSize , setIllustrationBarSize ] = useSafeState('50%')
    const [ visualizerBarSize , setVisualizerBarSize ] = useSafeState(process.env.REACT_APP_ENVIROMENT_MODE === 'STANDARD' ? '40.1%' : '0%')

    const [ assetTableFocus, setAssetTableFocus ] = useState( false )
    const [ companyButtonVisible, setCompanyButtonVisible ] = useSafeState(false)
    const [ typeButtonVisible, setTypeButtonVisible ] = useSafeState(false)
    const [ otherPartyButtonVisible, setOtherPartyButtonVisible ] = useSafeState(false)
    const [ assignmentButtonVisible, setAssignmentButtonVisible ] = useSafeState(false)
    const [ customerButtonVisible, setCustomerButtonVisible ] = useSafeState(false)

    const [ assetFilesBarSize, setAssetFilesBarSize ] = useSafeState(0)
    const [ assetFilesBar, setAssetFilesBar ] = useSafeState(false)
    const [ openGoogleDriveBar, setGoogleDriveBar] = useSafeState(false)
    const [ toggleAssetFileButtonType, setToggleAssetFileButtonType ] = useSafeState(true)

    const [ driveTemplateBarSize, setDriveTemplateBarSize ] = useSafeState(200)

    const [ isDrag, setIsDrag ] = useState(false)
    const [ request, setRequest ] = useSafeState(false)
