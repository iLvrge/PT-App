...
const LegalEventsContainer = ({ events, type, standalone, activeTab }) => {
  ...
  const ItemLabel = ({label}) =>  {
    return (
        label === 'M.Fees'
        ?
          <span className={clsx('pt-legal-container-relative', {['pt-red-color']: asset_details.fees > 20 ? true : false})}>{label}<Badge color='primary' max={99999} className={'pt-legal-badge'} badgeContent={numberWithCommas(asset_details.fees)} showZero={false}></Badge></span>
        :
            label === 'Cited by'
            ?
              <span className={clsx('pt-legal-container-relative', {['pt-red-color']: asset_details.citations > 20 ? true : false})}>{label}<Badge color='primary' max={99999} className={'pt-legal-badge'} badgeContent={numberWithCommas(asset_details.citations)} showZero={false}></Badge></span>
            :
                label === 'PTAB'
                ?
                  <span className={clsx('pt-legal-container-relative', {['pt-red-color']: asset_details.ptab > 20 ? true : false})}>{label}<Badge color='primary' max={99999} className={'pt-legal-badge'} badgeContent={numberWithCommas(asset_details.ptab)} showZero={false}></Badge></span>
                :
                  label == 'Litigation'
                  ?
                    <span className={clsx('pt-legal-container-relative', {['pt-red-color']: asset_details.litigation > 20 ? true : false})}>{label}<Badge color='primary' max={99999} className={'pt-legal-badge'} badgeContent={numberWithCommas(asset_details.litigation)} showZero={false}></Badge></span>
                  :
                  label == 'Status'
                  ?
                    <span className={clsx('pt-legal-container-relative', {['pt-red-color']: asset_details.status > 20 ? true : false})}>{label}<Badge color='primary' max={99999} className={'pt-legal-badge'} badgeContent={numberWithCommas(asset_details.status)} showZero={false}></Badge></span>
                  :
                  label
    )
}
          ...
          <>
            {
              fullScreen === false && typeof standalone === 'undefined' && (
                  <IconButton size="small" className={clsx('pt-legal-fullscreen-btn', 'full_screen_btn')} onClick={() => setFullScreen(!fullScreen)}>
                    <FullscreenIcon />
                  </IconButton>
              )
            }
            <Tabs className={'pt-legal-tabs'} variant={'scrollable'} value={selectedTab} onChange={handleChangeTab}>
              {
                [`M.Fees`, `Cited by`, `Status`, `PTAB`, `Litigation`].map( (item, index) => (
                  <Tab
                    key={index}
                    className={'pt-legal-tab'} 
                    label={<ItemLabel label={item}/>}
                  /> 
                ))
              }                            
            </Tabs>
            <div className={'pt-legal-graph-container'}>  
